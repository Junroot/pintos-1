...
argument_stack(char **parse, int count, void **esp)
{
	...
	for (i=0; i<count; ++i)
	{
		len = strlen(parse[count-i-1])+1;
		*esp = *esp - sizeof(char)*len;
		strlcpy((char*)(*esp), parse[count-i-1], len);
		if (arg_ptr == NULL)	arg_ptr=(*esp);
	}
	// 4 Bytes Alignment
	*esp = (void*) ((intptr_t)(*esp) & 0xfffffffc);
	*esp = (((char**) (*esp)) -1);
	*((char*)(*esp)) = 0;
	// Push pointer of argument to stack
	for (i=0; i<count; ++i)
	{
		*esp = *esp - sizeof(char**);
		*((char**)(*esp)) = arg_ptr;
...
bool handle_mm_fault(struct vm_entry *vme)
{
	...
	switch(vme->type)
	{
		case VM_BIN :
		case VM_FILE :
			success = load_file(addr, vme);
			break;
		case VM_ANON :
			swap_in(vme->swap_slot, addr);
			success = true;
			break;
		default :
			return false;
